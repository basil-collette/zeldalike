...
{
    public abstract class BaseRepository<T, M> : IReadRepository<T>, IWriteRepository<T>, IModelMaker<T, M>
        where T : DbDataScriptable /* T : data row class */
        where M : BaseDbData /* M : model */
    {
        const string DATA_FOLDER_PATH = "ScriptableObjects/Items";
        string _folderName;
        ...
        public T[] GetAll(bool isNested = false)
        {
            try
            {
                T[] rows = Resources.LoadAll<T>($"{DATA_FOLDER_PATH}/{_folderName}/");
        ...
        public T GetByCode(string code, bool isNested = false)
        {
            try
            {
                return Resources.Load<T>($"{DATA_FOLDER_PATH}/{_folderName}/{code}");
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
        ...
        public abstract M DbDataToModel(T data);
...
*/
    ...
    public interface IModelMaker<T, M>
        where T : DbDataScriptable // T : data
        where M : BaseDbData // M : model
    {
        M DbDataToModel(T data);
    }

    public interface IReadRepository<T> where T : DbDataScriptable // T : data row class
    {
        T[] GetAll(bool isNested = false);
        T GetByCode(string code, bool isNested = false);
        T GetById(int id, bool isNested = false);
    }

    public interface IWriteRepository<T> where T : DbDataScriptable // T : data row class
    {
